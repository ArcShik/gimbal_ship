...
/* ----------------------------- IMU_TEMPRETURE ----------------------------- */
#define IMU_TARGET_TEMP           50            /* imu期望恒温温度 */

static uint32_t period = 250000;
 uint32_t pulse = 0;
static pid_obj_t *imu_temp_pid;
static pid_config_t imu_temp_config = INIT_PID_CONFIG(100, 4, 2, 6, 700, PID_Integral_Limit);
...
__attribute__((noreturn)) void ins_task_entry(void const *argument)
{
    ...
    uint32_t ins_uxHighWaterMark=0;
...
//        ins_uxHighWaterMark = uxTaskGetStackHighWaterMark( NULL );
        dt = dwt_get_delta(&ins_dwt);
        BMI088_Read(&BMI088);
